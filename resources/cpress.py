...
PATH_IMAGES = '.\\image'
PATH_SOMEIMAGES = '.\\someImage'
PATH_TXT2IMG = '..\\data\\TXT2IMG'
PATH = PATH_IMAGES
...
def compress(filename, path):
    ...
    save_args['quality'], value = jpeg_dynamic_quality(im)
...
def walk_file(PATH):
    for root, dirs, files in os.walk(PATH):
        if dirs:
            for dir in dirs:
                walk_file(os.path.join(PATH, dir))
        else:
            for file in files:
                if re.findall('jpg', file) or re.findall('png', file):
                    print(os.path.join(PATH, file))
                    compress(file, os.path.join(PATH, file))
            return
