...
from .config import INTERACTION_PATH, SOMEIMAGE_PATH, AUTOREPLY_PATH
...
class Txt2Img:
    ...
    def __init__(self, size=30):
        ...
        self.save_dir_INTERACTION = str(INTERACTION_PATH)
        self.save_dir_SOMEIMAGE = str(SOMEIMAGE_PATH)
        self.save_dir_AUTOREPLY = str(AUTOREPLY_PATH)
    ...
    def save(self, title, lrc, type):
        ...
        draw_rectangle(
            draw, (out_padding, out_padding, w -
                   out_padding, h - out_padding), 2
        )
        ...
        out_img.paste(
            mi_banner.transpose(Image.FLIP_LEFT_RIGHT).transpose(
        ...
        self.save_img(out_img=out_img, type=type)
        return self.img2b64(out_img)

    def save_img(self, out_img, type):
        """save out_img object to local disk"""
        img_save_path = ""
        if type == "interaction.png":
            img_save_path = f"{self.save_dir_INTERACTION}/{type}"
        elif type == "someImage.png":
            img_save_path = f"{self.save_dir_SOMEIMAGE}/{type}"
        elif type == "autoReply.png":
            img_save_path = f"{self.save_dir_AUTOREPLY}/{type}"
