...
import re
Path_data = Path.cwd() / 'resources' / 'json'
Path_root = Path.cwd()

Bot_NICKNAME: str = '姐姐我'      # bot的nickname,可以换成你自己的
Bot_MASTER: str = "群主"      # bot的主人名称,也可以换成你自己的
# NICKNAME: str = "Hinata"
...
AnimeThesaurus = json.load(
    open(Path(os.path.join(Path_data)) / "data.json", "r", encoding="utf8"))


# 获取resource/audio下面的全部文件
aac_file_path = os.path.join(os.path.join(Path_root), "resources/audio")
...
async def get_chat_result(text: str, nickname: str) -> str:  # type:ignore
    if len(text) < 7:
